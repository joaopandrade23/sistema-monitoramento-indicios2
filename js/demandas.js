...
function estadoSemanticoCiclo(ciclo={}){
  ...
  return {concluidoOperacionalmente:concluido,encerradoDefinitivamente:definitivo,cicloVigente:ciclo.ciclo_ativo===true,permiteEdicao:editavel,somenteLeitura:!editavel};
}
function filtrosConcluidas(){return {p_busca:el.concluidaBusca.value.trim()||null,p_id_operador:gx("concluidaOperador").value?Number(gx("concluidaOperador").value):null,p_id_tipo_indicio:el.concluidaTipo.value?Number(el.concluidaTipo.value):null,p_codigo_status:gx("concluidaStatus").value||null,p_codigo_prioridade:gx("concluidaPrioridade").value||null,p_data_inicial:el.concluidaInicio.value||null,p_data_final:el.concluidaFim.value||null,p_ordenacao:gx("concluidaOrdenacao").value||"ENCERRAMENTO_DESC",p_pagina:estado.concluidas.pagina,p_tamanho_pagina:estado.concluidas.tamanho};}
async function carregarConcluidasGestor(){
  const e=gx("concluidasConteudo");e.innerHTML='<div class="table-state">Carregando demandas concluídas...</div>';
  try{
    const {data,error}=await sb.rpc("listar_demandas_concluidas_gestor",filtrosConcluidas());if(error)throw error;
    GX.concluidas=data?.itens||[];estado.concluidas.itens=GX.concluidas;const p=data?.paginacao||{};estado.concluidas.total=p.total_registros||0;estado.concluidas.totalPaginas=p.total_paginas||0;
    const comResp=GX.concluidas.filter(x=>x.nome_operador_principal).length,comProc=GX.concluidas.filter(x=>x.processo_sei_principal||x.quantidade_processos_sei).length;
    el.metricasConcluidas.innerHTML=[["Total concluído",p.total_registros||0],["Nesta página",GX.concluidas.length],["Com responsável histórico",comResp],["Com processo SEI",comProc],["Com colaboradores",GX.concluidas.filter(x=>x.quantidade_colaboradores>0).length],["Página",`${p.pagina||1}/${p.total_paginas||0}`]].map(([l,v])=>`<article class="metric"><span>${l}</span><strong>${v}</strong><small>Consulta histórica</small></article>`).join("");
    el.resumoConcluidas.textContent=`${p.total_registros||0} demanda(s) concluída(s) encontrada(s)`;el.infoConcluidas.textContent=GX.concluidas.length?`Exibindo ${GX.concluidas.length} registro(s) nesta página`:'Nenhum registro';gx("paginaConcluidasInfo").textContent=`Página ${p.pagina||1} de ${p.total_paginas||0}`;gx("concluidasAnteriorBtn").disabled=!p.possui_pagina_anterior;gx("concluidasProximaBtn").disabled=!p.possui_proxima_pagina;
    e.innerHTML=GX.concluidas.length?`<div class="table-wrap"><table class="concluded-table"><thead><tr><th class="concluded-select"><input type="checkbox" data-select-all-concluded aria-label="Selecionar página"></th><th></th><th>Indício</th><th>Pessoa</th><th>Tipo</th><th>Situação final</th><th>Responsável no encerramento</th><th>Participantes</th><th>Processo SEI</th><th>Encerramento</th><th>Ciclo</th><th>Ação</th></tr></thead><tbody>${GX.concluidas.map(x=>{const k=String(x.id_ciclo_tratamento||x.id_indicio),sel=estado.concluidas.selecionadas.has(k);return `<tr class="${sel?'is-selected':''}"><td><input type="checkbox" data-select-concluded="${k}" ${sel?'checked':''}></td><td><button class="btn btn-sm" data-expand-concluida="${x.id_indicio}" aria-label="Expandir">⌄</button></td><td><strong>${escapeHtml(x.identificador_do_indicio)}</strong></td><td>${escapeHtml(x.nome_atual)}<br><small>${escapeHtml(x.cpf_mascarado)}</small></td><td>${escapeHtml(x.tipo_indicio)}</td><td><span class="badge ${classeSituacao(x.codigo_status_ciclo)}">${escapeHtml(x.nome_status_ciclo||rotuloSituacao(x.codigo_status_ciclo))}</span></td><td>${escapeHtml(x.nome_operador_principal,"Consultar ciclo")}</td><td>${Number(x.quantidade_colaboradores||0)} colaborador(es)</td><td>${escapeHtml(x.processo_sei_principal,"Nenhum informado")}</td><td>${formatarDataHora(x.encerrado_em)}</td><td>${x.numero_ciclo||"-"}</td><td><button class="btn btn-secondary" data-concluida="${x.id_indicio}">Detalhes</button></td></tr><tr class="concluded-row-detail" data-detail-concluida="${x.id_indicio}" hidden><td colspan="12"><div class="concluded-expansion"><article><h3>Resumo do ciclo</h3><p>${escapeHtml(x.nome_status_ciclo)}</p><p>Resultado: ${escapeHtml(x.resultado_encerramento)}</p></article><article><h3>Participantes históricos</h3><p>${escapeHtml(x.nome_operador_principal,"Abra os detalhes para consultar.")}</p></article><article><h3>Processos SEI</h3><p>${escapeHtml(x.processo_sei_principal,"Nenhum processo principal informado.")}</p></article><article><h3>Auditoria</h3><p>${Number(x.quantidade_movimentacoes||0)} movimentação(ões). Somente leitura.</p></article></div></td></tr>`}).join("")}</tbody></table></div>`:'<div class="table-state">Nenhuma demanda concluída.</div>';
  }catch(x){console.error(x);e.innerHTML=`<div class="status-banner error">${escapeHtml(mensagemErro(x,x.message||"Não foi possível carregar as concluídas."))}</div>`}
}
function linhasCsv(payload){const cols=payload?.colunas||[];return [cols,...(payload?.itens||[]).map(item=>cols.map(c=>item[c]))];}
async function exportarDemandasRpc(concluidas=false){
  const scope=gx(concluidas?"escopoExportacaoConcluidas":"escopoExportacaoAtuais").value;
  const selected=concluidas?[...estado.concluidas.selecionadas.values()]:[...estado.selecionadas.values()];
  if(scope==="SELECIONADOS"&&!selected.length){exibirMensagem("Selecione ao menos uma demanda para exportar.","warning");return}
  const f=concluidas?filtrosConcluidas():parametrosListagem();
  const args={p_escopo:scope,p_ids_indicios:scope==="SELECIONADOS"?selected.map(x=>Number(x.id_indicio)):null,p_ids_ciclos:scope==="SELECIONADOS"?selected.map(x=>Number(x.id_ciclo_tratamento)).filter(Boolean):null,p_apenas_concluidas:concluidas,p_busca:f.p_busca||null,p_id_operador:f.p_id_operador||null,p_id_tipo_indicio:f.p_id_tipo_indicio||null,p_codigo_status:concluidas?(f.p_codigo_status||null):(f.p_situacao_operacional||null),p_codigo_prioridade:f.p_codigo_prioridade||null,p_data_inicial:concluidas?f.p_data_inicial:null,p_data_final:concluidas?f.p_data_final:null,p_pagina:concluidas?estado.concluidas.pagina:estado.paginacao.pagina,p_tamanho_pagina:concluidas?estado.concluidas.tamanho:estado.paginacao.tamanho};
  try{const {data,error}=await sb.rpc("exportar_demandas_gestor",args);if(error)throw error;if(!data?.itens?.length){exibirMensagem("Nenhum registro disponível para exportação.","warning");return}gcsv(linhasCsv(data),`demandas_${concluidas?'concluidas':'atuais'}_${scope.toLowerCase()}.csv`);exibirMensagem(`${data.quantidade_registros} registro(s) exportado(s).`,"success")}catch(e){exibirMensagem(mensagemErro(e,e.message||"Não foi possível exportar."),"error")}
}
async function carregarCiclosNoDetalhe(idIndicio,idSelecionado){
  const {data,error}=await sb.rpc("listar_ciclos_demanda_gestor",{p_id_indicio:Number(idIndicio)});if(error)throw error;const ciclos=data?.itens||[];if(!ciclos.length)return;
  const atual=ciclos.find(x=>Number(x.id_ciclo_tratamento)===Number(idSelecionado))||ciclos[0];
  const selector=`<h3 class="section-title">Ciclos da demanda</h3><div class="cycle-selector">${ciclos.map(c=>{const st=estadoSemanticoCiclo(c);return `<button class="cycle-card ${Number(c.id_ciclo_tratamento)===Number(atual.id_ciclo_tratamento)?'active':''}" data-cycle-report="${c.id_ciclo_tratamento}"><header><strong>Ciclo ${c.numero_ciclo}</strong><span class="badge">${escapeHtml(c.nome_status_ciclo)}</span></header><small>${formatarDataHora(c.aberto_em)} a ${formatarDataHora(c.encerrado_em)}</small><span>${st.cicloVigente?'Ciclo vigente':'Ciclo histórico'} · ${st.somenteLeitura?'Somente leitura':'Editável'}</span></button>`}).join("")}</div>`;
  el.painelRelatorioGestor.insertAdjacentHTML("afterbegin",selector);
}
async function exportarRelatorioRpc(integral=false,idCiclo=null){
  const item=estado.detalhe.demanda;if(!item)return;try{const chamada=integral?sb.rpc("relatorio_historico_demanda_gestor",{p_id_indicio:Number(item.id_indicio)}):sb.rpc("relatorio_ciclo_demanda_gestor",{p_id_ciclo_tratamento:Number(idCiclo||estado.detalhe.dados?.id_ciclo_selecionado||item.id_ciclo_tratamento)});const {data,error}=await chamada;if(error)throw error;estado.detalhe.dadosRelatorio=data;estado.detalhe.historico=data.historico||[];if(integral&&data.demanda)estado.detalhe.dados={...estado.detalhe.dados,...data.demanda,ciclos:data.ciclos};else if(data.ciclo)estado.detalhe.dados={...estado.detalhe.dados,ciclo_selecionado:data.ciclo};exportarRelatorioGestor()}catch(e){exibirMensagem(mensagemErro(e,e.message||"Não foi possível gerar o relatório."),"error")}
}
function conectarIntegracaoFinal(){
  gx("concluidaOperador").innerHTML='<option value="">Todos</option>'+estado.operadores.map(o=>`<option value="${o.id_usuario}">${escapeHtml(o.nome_exibicao)}</option>`).join("");gx("concluidaPrioridade").innerHTML='<option value="">Todas</option>'+estado.prioridades.map(p=>`<option value="${p.codigo_prioridade}">${escapeHtml(p.nome_prioridade)}</option>`).join("");el.concluidaTipo.innerHTML='<option value="">Todos</option>'+estado.tiposIndicio.map(t=>`<option value="${t.id}">${escapeHtml(t.nome)}</option>`).join("");
  gx("tamanhoPaginaConcluidas").onchange=e=>{estado.concluidas.tamanho=Number(e.target.value);estado.concluidas.pagina=1;carregarConcluidasGestor()};gx("concluidasAnteriorBtn").onclick=()=>{if(estado.concluidas.pagina>1){estado.concluidas.pagina--;carregarConcluidasGestor()}};gx("concluidasProximaBtn").onclick=()=>{if(estado.concluidas.pagina<estado.concluidas.totalPaginas){estado.concluidas.pagina++;carregarConcluidasGestor()}};
  el.exportarAtuaisBtn.onclick=()=>exportarDemandasRpc(false);el.exportarConcluidasBtn.onclick=()=>exportarDemandasRpc(true);el.exportarRelatorioGestorBtn.onclick=()=>exportarRelatorioRpc(true);el.painelRelatorioGestor.onclick=e=>{const c=e.target.closest("[data-cycle-report]");if(c)exportarRelatorioRpc(false,c.dataset.cycleReport);else if(e.target.closest("[data-export-report]"))exportarRelatorioRpc(true)};
  gx("concluidasConteudo").addEventListener("change",e=>{if(e.target.matches("[data-select-concluded]")){const x=GX.concluidas.find(r=>String(r.id_ciclo_tratamento||r.id_indicio)===e.target.dataset.selectConcluded);if(x){if(e.target.checked)estado.concluidas.selecionadas.set(e.target.dataset.selectConcluded,x);else estado.concluidas.selecionadas.delete(e.target.dataset.selectConcluded)}carregarConcluidasGestor()}else if(e.target.matches("[data-select-all-concluded]")){GX.concluidas.forEach(x=>{const k=String(x.id_ciclo_tratamento||x.id_indicio);if(e.target.checked)estado.concluidas.selecionadas.set(k,x);else estado.concluidas.selecionadas.delete(k)});carregarConcluidasGestor()}});
  const originalAbrir=abrirDetalhe;abrirDetalhe=async function(d){await originalAbrir(d);if(estado.detalhe.dados)try{await carregarCiclosNoDetalhe(d.id_indicio,estado.detalhe.dados.id_ciclo_selecionado||d.id_ciclo_tratamento)}catch(e){console.error(e)}};
}
const inicializarBase=inicializarAplicacao;
inicializarAplicacao=async function(){await inicializarBase();conectarIntegracaoFinal();};
