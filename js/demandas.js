...
function switchDetailTab(tab){document.querySelectorAll('[data-detail-tab]').forEach(b=>b.classList.toggle('active',b.dataset.detailTab===tab));document.querySelectorAll('[data-detail-panel]').forEach(p=>p.hidden=p.dataset.detailPanel!==tab)}
function rotuloGestao(v){return ({PENDENTE:'Pendente de tratamento',PENDENTE_DE_TRATAMENTO:'Pendente de tratamento',DISPONIVEL_PARA_ATRIBUICAO:'Disponível para atribuição',EM_TRATAMENTO:'Em tratamento',ENCERRADO_INTERNAMENTE:'Encerrado internamente',VENCE_EM_ATE_3_DIAS:'Vence em até 3 dias',VENCE_EM_ATE_7_DIAS:'Vence entre 4 e 7 dias',SEM_PRAZO:'Sem prazo',NAO_SE_APLICA:'Não se aplica',PRAZO_VENCIDO:'Prazo vencido',DENTRO_DO_PRAZO:'Dentro do prazo'})[v]||String(v||'Não informado').replaceAll('_',' ').toLowerCase().replace(/(^|\s)\S/g,m=>m.toUpperCase())}
async function rpcDetalheSeguro(id){const tentativas=[['obter_detalhes_demanda_modo',{p_id_indicio:Number(id)}],['obter_detalhes_demanda',{p_id_indicio:Number(id)}]];let ultimo;for(const [n,a] of tentativas){try{const{data,error}=await sb.rpc(n,a);if(error)throw error;return data}catch(e){ultimo=e}}throw ultimo}
async function abrirDetalhe(d) {
  if(!d)return;estado.detalhe.demanda=d;estado.detalhe.dados=null;el.gerenciarEquipeBtn.hidden=true;const req=++estado.detalhe.requisicao;
  el.modalIdentificador.textContent=d.identificador_do_indicio||"Não informado";el.modalNome.textContent=d.nome_atual||"Não informado";el.modalCpf.textContent=d.cpf_mascarado||"CPF protegido";el.modalTipo.textContent=d.tipo_indicio||"Não informado";el.modalSituacao.textContent=rotuloSituacao(d.situacao_operacional);el.modalSituacao.className=`badge ${classeSituacao(d.situacao_operacional)}`;el.modalPrioridade.textContent=d.nome_prioridade||"Não definida";el.modalPrazo.textContent=d.prazo_em?formatarDataHora(d.prazo_em):"Sem prazo";el.modalOperador.textContent=d.nome_operador_principal||"Carregando histórico do ciclo...";el.modalProcessosQtd.textContent="...";el.painelDetalhesGestor.innerHTML='<div class="table-state">Carregando detalhes...</div>';el.painelEquipeGestor.innerHTML='<div class="table-state">Carregando equipe...</div>';el.painelProcessosGestor.innerHTML='<div class="table-state">Carregando processos...</div>';el.painelHistoricoGestor.innerHTML='<div class="table-state">Carregando histórico...</div>';el.atribuicaoOverlay.hidden=false;document.body.style.overflow="hidden";switchDetailTab('detalhes');
  try{
    const data=await rpcDetalheSeguro(d.id_indicio);if(req!==estado.detalhe.requisicao)return;estado.detalhe.dados=data;const ciclo=data.ciclo_selecionado||data;const processos=data.processos_sei||data.processos||[];const equipe=ciclo.equipe||data.equipe||[];const principal=data.operador_principal||equipe.find(x=>x.papel_principal&&x.participacao_ativa)||{};const colaboradores=data.colaboradores||equipe.filter(x=>!x.papel_principal&&x.participacao_ativa);
    el.modalCpf.textContent=data.cpf_mascarado||data.cpf||d.cpf_mascarado||"Não informado";el.modalNome.textContent=data.nome_atual||d.nome_atual;el.modalTipo.textContent=data.tipo_indicio||d.tipo_indicio;el.modalPrioridade.textContent=ciclo.nome_prioridade||data.nome_prioridade||d.nome_prioridade||"Não definida";el.modalPrazo.textContent=(ciclo.prazo_em||data.prazo_em)?formatarDataHora(ciclo.prazo_em||data.prazo_em):"Sem prazo";el.modalOperador.textContent=principal.nome_exibicao||d.nome_operador_principal||"Responsável histórico não informado";el.modalProcessosQtd.textContent=processos.filter(x=>x.processo_ativo!==false).length;
    el.painelDetalhesGestor.innerHTML=detailSection('Identificação',detailCard('Número do indício',data.identificador_do_indicio||d.identificador_do_indicio)+detailCard('Base de dados',data.base_de_dados||d.base_de_dados)+detailCard('Tipo de indício',data.tipo_indicio||d.tipo_indicio,'full')+detailCard('Descrição',data.descricao_indicio||'Descrição não informada.','full'))+detailSection('Pessoa',detailCard('Nome atual',data.nome_atual||d.nome_atual,'wide')+detailCard('CPF',data.cpf_mascarado||data.cpf||d.cpf_mascarado)+detailCard('Situação funcional',data.situacoes_funcionais_resumo||d.situacoes_funcionais_resumo,'full'));
    el.painelEquipeGestor.innerHTML=(ciclo.ciclo_encerrado||d.situacao_operacional==='ENCERRADO_INTERNAMENTE'?'<div class="readonly-callout"><strong>Modo somente leitura.</strong> Este ciclo está concluído e não permite movimentações.</div>':'')+detailSection('Ciclo',detailCard('Número do ciclo',ciclo.numero_ciclo)+detailCard('Situação',ciclo.nome_status_ciclo||rotuloSituacao(d.situacao_operacional))+detailCard('Versão',ciclo.versao_ciclo||ciclo.versao)+detailCard('Modo de trabalho',data.modo_trabalho?.nome||data.modo_trabalho?.codigo||'Não informado')+detailCard('Responsável principal',principal.nome_exibicao||d.nome_operador_principal||'Não informado','wide')+detailCard('Colaboradores',colaboradores.map(x=>x.nome_exibicao).join(', ')||'Nenhum colaborador','full'));
    el.painelProcessosGestor.innerHTML=processos.length?`<div class="process-list">${processos.map(x=>`<article class="process-detail-card ${x.processo_ativo===false?'inactive':''}"><h3>${escapeHtml(x.numero_processo)} ${x.processo_principal?'<span class="badge badge-primary">Principal</span>':''}</h3><p>${escapeHtml(x.assunto||'Sem assunto')}</p><small>Incluído em ${formatarDataHora(x.incluido_em)}${x.processo_ativo===false?`<br>Inativado em ${formatarDataHora(x.inativado_em)}. Motivo: ${escapeHtml(x.motivo_inativacao||'Não informado')}`:''}</small></article>`).join('')}</div>`:'<div class="table-state">Nenhum processo SEI vinculado ao ciclo.</div>';
    try{const hist=await rpcGestao('listar_movimentacoes_demanda_gestor',{p_id_indicio:Number(d.id_indicio),p_id_ciclo_tratamento:ciclo.id_ciclo_tratamento||d.id_ciclo_tratamento||null,p_categoria:null,p_data_inicial:null,p_data_final:null,p_pagina:1,p_tamanho_pagina:200});const rows=hist?.itens||[];el.painelHistoricoGestor.innerHTML=rows.length?`<div class="timeline">${rows.map(x=>`<article class="timeline-item"><span class="history-icon">${x.evento_automatico?'A':'H'}</span><div><div class="history-heading"><strong>${escapeHtml(x.nome_movimentacao||x.codigo_movimentacao||'Movimentação')}</strong><time>${formatarDataHora(x.realizada_em)}</time></div><p>${escapeHtml(x.descricao||'Sem descrição.')}</p><small>${escapeHtml(x.nome_executor||x.executor?.nome_exibicao||'')}</small></div></article>`).join('')}</div>`:'<div class="table-state">Nenhuma movimentação registrada.</div>'}catch(h){el.painelHistoricoGestor.innerHTML=`<div class="status-banner warning">Não foi possível carregar o histórico: ${escapeHtml(h.message)}</div>`}
    el.gerenciarEquipeBtn.hidden=Boolean(ciclo.ciclo_encerrado||d.situacao_operacional==='ENCERRADO_INTERNAMENTE'||!data.possui_ciclo_ativo);
  }catch(error){console.error(error);if(req!==estado.detalhe.requisicao)return;el.modalMensagemDetalhe.hidden=false;el.modalMensagemDetalhe.className='status-banner modal-message error';el.modalMensagemDetalhe.textContent=mensagemErro(error,'Não foi possível carregar os detalhes completos.');el.painelDetalhesGestor.innerHTML=detailSection('Dados disponíveis',detailCard('Indício',d.identificador_do_indicio)+detailCard('Pessoa',d.nome_atual)+detailCard('CPF',d.cpf_mascarado)+detailCard('Tipo',d.tipo_indicio,'full'))}
}
...
function exportarAtuaisGestao(){const c=v=>`"${String(v??"").replaceAll('"','""')}"`;const headers=["Indício","Pessoa","CPF","Tipo","Vínculos","Situação","Prioridade","Operador","Prazo"];const rows=estado.demandas.map(x=>[x.identificador_do_indicio,x.nome_atual,x.cpf_mascarado,x.tipo_indicio,x.situacoes_funcionais_resumo,rotuloSituacao(x.situacao_operacional),x.nome_prioridade,x.nome_operador_principal,x.prazo_em?formatarDataHora(x.prazo_em):"Sem prazo"]);const a=document.createElement("a");a.href=URL.createObjectURL(new Blob(["\ufeff"+[headers,...rows].map(r=>r.map(c).join(";")).join("\r\n")],{type:"text/csv;charset=utf-8"}));a.download="demandas_atuais_filtradas.csv";a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
