...
/* Frontend final do Gestor SEGEP/CE: painel, abas, historico e administracao */
const GS={tab:"painel",painel:null,concluidas:[],historico:[],admin:null};
const gid=id=>document.getElementById(id);
const gRpc=async(name,args={})=>{const{data,error}=await sb.rpc(name,args);if(error)throw error;return data};
const gMsg=(text,type="")=>{const x=gid("adminMensagem");if(!x)return;x.hidden=!text;x.textContent=text||"";x.className=`status-banner modal-message ${type}`};
const gCsv=(name,headers,rows)=>{const c=v=>`"${String(v??"").replaceAll('"','""')}"`;const body="\ufeff"+[headers,...rows].map(r=>r.map(c).join(";")).join("\r\n");const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([body],{type:"text/csv;charset=utf-8"}));a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)};
function gBars(target,data,label){const e=gid(target);if(!e)return;if(!data?.length){e.innerHTML='<div class="empty-chart">Sem dados no periodo.</div>';return}const max=Math.max(...data.map(x=>+x.quantidade),1);e.innerHTML='<div class="bar-chart">'+data.map(x=>`<div class="bar-row"><span class="bar-label" title="${escapeHtml(x[label])}">${escapeHtml(x[label])}</span><span class="bar-track"><span class="bar-fill" style="width:${Math.max((+x.quantidade/max)*100,3)}%"></span></span><span class="bar-value">${+x.quantidade}</span></div>`).join("")+"</div>"}
function gColumns(data){const e=gid("graficoConclusoesGestao");if(!e)return;if(!data?.length){e.innerHTML='<div class="empty-chart">Sem conclusoes no periodo.</div>';return}const max=Math.max(...data.map(x=>+x.quantidade),1),step=Math.max(1,Math.ceil(data.length/8));e.innerHTML='<div class="daily-chart">'+data.map((x,i)=>`<div class="daily-column" style="height:${Math.max((+x.quantidade/max)*100,2)}%"><span>${x.quantidade||""}</span>${i%step===0||i===data.length-1?`<time>${formatarData(x.data).slice(0,5)}</time>`:""}</div>`).join("")+"</div>"}
async function carregarPainelGestao(){try{const data=await gRpc("resumo_painel_gestor_segep",{p_data_inicial:gid("painelInicio").value||null,p_data_final:gid("painelFim").value||null});GS.painel=data;const c=data.cards||{};gid("metricasPainelGestao").innerHTML=[["Total de demandas",c.total_demandas],["Sem responsavel",c.sem_responsavel],["Aguardando inicio",c.aguardando_inicio],["Em analise",c.em_analise],["Prazo proximo",c.prazo_proximo],["Prazo vencido",c.prazo_vencido]].map(([l,v],i)=>`<article class="metric metric-${i}"><span>${l}</span><strong>${v??0}</strong><small>Estoque atual</small></article>`).join("");gBars("graficoStatusGestao",data.por_status,"nome_status");gBars("graficoPrazosGestao",data.por_prazo,"faixa");gBars("graficoTiposGestao",data.por_tipo_indicio,"tipo_indicio");gBars("graficoMovimentacoesGestao",data.movimentacoes_administrativas,"nome_movimentacao");gColumns(data.concluidas_por_dia);const cargas=data.carga_operadores||[];gid("cargaOperadoresGestao").innerHTML=cargas.length?cargas.map(x=>`<article class="operator-load-card"><header><div><strong>${escapeHtml(x.nome_exibicao)}</strong><small>${escapeHtml(x.email_institucional,"")}</small></div><span class="badge badge-primary">${x.carga?.total_participacoes_ativas??0}</span></header><div class="load-metrics"><div><b>${x.carga?.como_principal??0}</b><small>Principal</small></div><div><b>${x.carga?.como_colaborador??0}</b><small>Colaborador</small></div><div><b>${x.situacao_principal?.prazos_vencidos??0}</b><small>Vencidas</small></div></div></article>`).join(""):'<div class="empty-chart">Nenhum operador disponivel.</div>'}catch(e){exibirMensagem(mensagemErro(e,e.message||"Nao foi possivel carregar o painel."),"error")}}
async function carregarConcluidasGestao(){const box=gid("concluidasConteudo");box.innerHTML="<strong>Carregando...</strong>";try{const data=await gRpc("listar_demandas_gestao",{...parametrosListagem(),p_situacao_operacional:"ENCERRADO_INTERNAMENTE",p_pagina:1,p_tamanho_pagina:100});GS.concluidas=data?.itens||[];box.innerHTML=GS.concluidas.length?`<div class="table-wrap"><table><thead><tr><th>Indicio</th><th>Pessoa</th><th>Tipo</th><th>Situacao</th><th>Operador</th><th>Acao</th></tr></thead><tbody>${GS.concluidas.map(x=>`<tr><td>${escapeHtml(x.identificador_do_indicio)}</td><td>${escapeHtml(x.nome_atual)}<br><small>${escapeHtml(x.cpf_mascarado)}</small></td><td>${escapeHtml(x.tipo_indicio)}</td><td><span class="badge">${escapeHtml(rotuloSituacao(x.situacao_operacional))}</span></td><td>${escapeHtml(x.nome_operador_principal,"Sem responsavel")}</td><td><button class="btn btn-secondary" data-concluida="${x.id_indicio}">Consultar</button></td></tr>`).join("")}</tbody></table></div>`:'<div class="table-state">Nenhuma demanda concluida encontrada.</div>'}catch(e){box.innerHTML=`<div class="status-banner error">${escapeHtml(e.message)}</div>`}}
async function carregarHistoricoGestao(){const box=gid("historicoGestaoLista");box.innerHTML='<div class="table-state">Carregando historico...</div>';try{let data;try{data=await gRpc("listar_movimentacoes_gestao",{p_busca:gid("historicoBusca").value||null,p_pagina:1,p_tamanho_pagina:200})}catch{data=await gRpc("listar_historico_demandas_gestao",{p_busca:gid("historicoBusca").value||null,p_pagina:1,p_tamanho_pagina:200})}GS.historico=data?.itens||data||[];box.innerHTML=GS.historico.length?GS.historico.map(x=>`<article class="history-entry"><span class="history-entry-icon">${x.evento_automatico?"A":"H"}</span><div><header><strong>${escapeHtml(x.nome_movimentacao||x.codigo_movimentacao||"Movimentacao")}</strong><time>${formatarDataHora(x.realizada_em||x.criado_em)}</time></header><p>${escapeHtml(x.descricao||"Sem descricao.")}</p><small>${escapeHtml(x.nome_executor||x.usuario_executor||"")}</small></div></article>`).join(""):'<div class="table-state">Nenhuma movimentacao encontrada.</div>'}catch(e){box.innerHTML=`<div class="status-banner warning">O contrato de historico integral ainda nao retornou dados: ${escapeHtml(e.message)}</div>`}}
function selecionarTabGestao(tab){GS.tab=tab;document.querySelectorAll("[data-gestao-tab]").forEach(b=>{const a=b.dataset.gestaoTab===tab;b.classList.toggle("active",a);b.setAttribute("aria-selected",String(a))});gid("secaoPainelGestao").hidden=tab!=="painel";gid("secaoDemandasAtuais").hidden=tab!=="atuais";gid("secaoConcluidasGestao").hidden=tab!=="concluidas";gid("secaoHistoricoGestao").hidden=tab!=="historico";if(tab==="painel")carregarPainelGestao();if(tab==="concluidas")carregarConcluidasGestao();if(tab==="historico")carregarHistoricoGestao()}
function adminField(l,v){return`<div class="audit-field"><span>${escapeHtml(l)}</span><strong>${escapeHtml(v)}</strong></div>`}
async function abrirAdministracao(d){try{GS.admin={demanda:d,dados:await gRpc("obter_detalhes_demanda_modo",{p_id_indicio:Number(d.id_indicio)})};const x=GS.admin.dados;gid("adminTitulo").textContent=`Indicio ${d.identificador_do_indicio}`;gid("adminDetalhes").innerHTML=`<div class="audit-grid">${adminField("Pessoa",x.nome_atual)}${adminField("CPF",x.cpf_mascarado||d.cpf_mascarado)}${adminField("Tipo",x.tipo_indicio)}${adminField("Situacao",rotuloSituacao(x.situacao_operacional))}${adminField("Prioridade",x.nome_prioridade||"Sem prioridade")}${adminField("Prazo",x.prazo_em?formatarDataHora(x.prazo_em):"Sem prazo")}${adminField("Principal",x.operador_principal?.nome_exibicao||"Sem responsavel")}${adminField("Modo",x.modo_trabalho?.nome||x.modo_trabalho?.codigo)}</div>`;gid("adminPrioridade").innerHTML=estado.prioridades.map(p=>`<option value="${p.id_prioridade||p.codigo_prioridade}">${escapeHtml(p.nome_prioridade)}</option>`).join("");gid("adminProcessos").innerHTML='<div class="table-state">Carregando processos...</div>';gid("adminHistorico").innerHTML='<div class="table-state">O historico da demanda sera exibido conforme o contrato gerencial.</div>';gid("administracaoOverlay").hidden=false;document.body.style.overflow="hidden";switchAdminTab("detalhes");renderAdminProcessos(x.processos_sei||x.processos||[])}catch(e){exibirMensagem(mensagemErro(e,e.message||"Nao foi possivel abrir a administracao."),"error")}}
function renderAdminProcessos(all){gid("adminProcessos").innerHTML=`<div class="table-toolbar"><strong>Processos SEI</strong><button class="btn btn-primary" id="novoProcessoGestorBtn">+ Vincular processo</button></div><div>${all.length?all.map(p=>`<article class="process-card ${p.processo_ativo===false?"inactive":""}"><div><strong>${escapeHtml(p.numero_processo)}</strong> ${p.processo_principal?'<span class="badge badge-primary">Principal</span>':""}<p>${escapeHtml(p.assunto||"Sem assunto")}</p></div><div class="process-actions">${p.processo_ativo!==false&&!p.processo_principal?`<button class="btn" data-admin-main="${p.id_processo_sei}">Definir principal</button>`:""}${p.processo_ativo!==false?`<button class="btn btn-danger" data-admin-inactivate="${p.id_processo_sei}">Inativar</button>`:""}</div></article>`).join(""):'<div class="table-state">Nenhum processo vinculado.</div>'}</div>`}
function switchAdminTab(tab){document.querySelectorAll("[data-admin-tab]").forEach(b=>b.classList.toggle("active",b.dataset.adminTab===tab));document.querySelectorAll("[data-admin-panel]").forEach(p=>p.hidden=p.dataset.adminPanel!==tab)}
function fecharAdmin(){gid("administracaoOverlay").hidden=true;document.body.style.overflow="";GS.admin=null}
async function salvarPrioridadePrazo(){try{const d=GS.admin?.dados,ciclo=d?.ciclo_selecionado||d;if(!ciclo?.id_ciclo_tratamento)throw Error("Ciclo ativo nao localizado.");const just=gid("adminJustificativa").value.trim();if(just.length<10)throw Error("Informe uma justificativa com pelo menos 10 caracteres.");const prioridade=estado.prioridades.find(p=>String(p.id_prioridade||p.codigo_prioridade)===gid("adminPrioridade").value);const prazo=gid("adminRemoverPrazo").checked?null:(gid("adminPrazo").value?`${gid("adminPrazo").value}T23:59:59-03:00`:null);await gRpc("alterar_prioridade_prazo_demanda_gestor",{p_id_ciclo_tratamento:Number(ciclo.id_ciclo_tratamento),p_versao_esperada:Number(ciclo.versao_ciclo||ciclo.versao),p_id_prioridade:prioridade?.id_prioridade?Number(prioridade.id_prioridade):null,p_codigo_prioridade:prioridade?.codigo_prioridade||null,p_prazo_em:prazo,p_remover_prazo:gid("adminRemoverPrazo").checked,p_justificativa:just});gMsg("Prioridade e prazo atualizados.","success");await Promise.all([carregarResumo(),carregarDemandas(),carregarPainelGestao()])}catch(e){gMsg(e.message,"error")}}
function imprimirRelatorioGestor(){if(!GS.admin)return;const d=GS.admin.dados,base=GS.admin.demanda;const html=`<!doctype html><html><head><meta charset="utf-8"><title>Relatorio ${escapeHtml(base.identificador_do_indicio)}</title><style>@page{size:A4;margin:16mm}body{font:10pt Arial;color:#172033}h1{color:#155eef}.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.f{border:1px solid #d0d5dd;padding:8px;border-radius:5px}.f span{display:block;color:#667085;font-size:8pt;text-transform:uppercase}.section{margin:16px 0}.section h2{font-size:13pt;border-bottom:1px solid #b9c8dd;padding-bottom:5px}</style></head><body><h1>Relatorio detalhado de auditoria da demanda</h1><p>Gerado em ${new Date().toLocaleString("pt-BR")}</p><div class="section"><h2>Identificacao</h2><div class="grid"><div class="f"><span>Indicio</span>${escapeHtml(base.identificador_do_indicio)}</div><div class="f"><span>Pessoa</span>${escapeHtml(d.nome_atual)}</div><div class="f"><span>CPF</span>${escapeHtml(d.cpf_mascarado||base.cpf_mascarado)}</div><div class="f"><span>Tipo</span>${escapeHtml(d.tipo_indicio)}</div></div></div><div class="section"><h2>Informacoes operacionais</h2><div class="grid"><div class="f"><span>Situacao</span>${escapeHtml(rotuloSituacao(d.situacao_operacional))}</div><div class="f"><span>Prioridade</span>${escapeHtml(d.nome_prioridade||"Sem prioridade")}</div><div class="f"><span>Principal</span>${escapeHtml(d.operador_principal?.nome_exibicao||"Sem responsavel")}</div><div class="f"><span>Modo</span>${escapeHtml(d.modo_trabalho?.nome||d.modo_trabalho?.codigo)}</div></div></div><p>Documento gerado pelo Sistema de Monitoramento de Indicios para apoio a consulta e auditoria.</p></body></html>`;const f=document.createElement("iframe");f.style.cssText="position:fixed;width:1px;height:1px;border:0";document.body.appendChild(f);f.contentDocument.open();f.contentDocument.write(html);f.contentDocument.close();setTimeout(()=>{f.contentWindow.print();setTimeout(()=>f.remove(),2000)},350)}
function initGestaoFinal(){const end=new Date(),start=new Date(Date.now()-29*86400000);gid("painelFim").value=end.toISOString().slice(0,10);gid("painelInicio").value=start.toISOString().slice(0,10);document.querySelectorAll("[data-gestao-tab]").forEach(b=>b.onclick=()=>selecionarTabGestao(b.dataset.gestaoTab));gid("aplicarPainelBtn").onclick=carregarPainelGestao;gid("carregarHistoricoBtn").onclick=carregarHistoricoGestao;gid("exportarConcluidasBtn").onclick=()=>gCsv("demandas_concluidas.csv",["Indicio","Pessoa","CPF","Tipo","Situacao"],GS.concluidas.map(x=>[x.identificador_do_indicio,x.nome_atual,x.cpf_mascarado,x.tipo_indicio,rotuloSituacao(x.situacao_operacional)]));gid("exportarHistoricoBtn").onclick=()=>gCsv("historico_integral.csv",["Data","Movimentacao","Descricao","Executor"],GS.historico.map(x=>[formatarDataHora(x.realizada_em||x.criado_em),x.nome_movimentacao||x.codigo_movimentacao,x.descricao,x.nome_executor||x.usuario_executor]));gid("concluidasConteudo").onclick=e=>{const b=e.target.closest("[data-concluida]");if(b){const d=GS.concluidas.find(x=>String(x.id_indicio)===b.dataset.concluida);if(d)abrirAdministracao(d)}};document.querySelectorAll("[data-admin-tab]").forEach(b=>b.onclick=()=>switchAdminTab(b.dataset.adminTab));gid("fecharAdminBtn").onclick=gid("fecharAdminRodapeBtn").onclick=fecharAdmin;gid("administracaoOverlay").onclick=e=>{if(e.target===gid("administracaoOverlay"))fecharAdmin()};gid("salvarPrioridadePrazoBtn").onclick=salvarPrioridadePrazo;gid("imprimirRelatorioGestorBtn").onclick=imprimirRelatorioGestor;const old=abrirDetalhe;window.abrirAdministracaoGestor=abrirAdministracao;gid("demandasTbody").addEventListener("dblclick",e=>{const tr=e.target.closest("tr[data-row-indicio]");if(tr){const d=estado.demandas.find(x=>String(x.id_indicio)===tr.dataset.rowIndicio);if(d)abrirAdministracao(d)}});selecionarTabGestao("painel")}
setTimeout(initGestaoFinal,0);
