...
function renderHistoricoConsolidado(rows) {
  ...
  const ordenados = [...rows].sort((a,b) => new Date(b.realizada_em) - new Date(a.realizada_em));
  let diaAtual = "";
  const itens = ordenados.map(item => {
    const data = new Date(item.realizada_em);
    const dia = Number.isNaN(data.getTime()) ? "Data não informada" : data.toLocaleDateString("pt-BR", { day:"2-digit", month:"long", year:"numeric" });
    const divisor = dia !== diaAtual ? `<h3 class="history-day">${dia}</h3>` : "";
    diaAtual = dia;
    const meta = rotuloHistoricoGestor(item);
    const executor = item.nome_executor || item.executor?.nome_exibicao || item.email_executor || item.categoria || "Evento automático";
    return `${divisor}<article class="timeline-item ${meta[1]}"><span class="history-icon">${meta[2]}</span><div class="history-content"><div class="history-heading"><strong>${escapeHtml(meta[0])}</strong><time>${formatarDataHora(item.realizada_em)}</time></div><p>${escapeHtml(descricaoHistoricoGestor(item, meta[0]))}</p><small>${escapeHtml(executor)}</small></div></article>`;
  }).join("");
  return `<div class="history-filters"><div class="field"><label>Buscar no histórico</label><input class="control" placeholder="Descrição, executor ou processo SEI"></div><div class="field"><label>Categoria</label><select class="control"><option>Todas</option><option>Atividade humana</option><option>Ação administrativa</option><option>Evento automático</option><option>Atualização da origem</option></select></div><div class="field"><label>Tipo de registro</label><select class="control"><option>Todos</option></select></div><div class="field"><label>Ordenação</label><select class="control"><option>Mais recentes primeiro</option><option>Mais antigos primeiro</option></select></div><div class="field"><label>Data inicial</label><input class="control" type="date"></div><div class="field"><label>Data final</label><input class="control" type="date"></div></div><div class="history-summary-line"><strong>${rows.length} ${rows.length === 1 ? "movimentação encontrada" : "movimentações encontradas"}</strong><span>Histórico do ciclo selecionado</span></div><div class="timeline">${itens}</div>`;
}
