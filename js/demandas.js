...
function renderizarPrevia(x) {
  const r=x.resumo||{}, elegiveis=Number(r.quantidade_elegivel||0), bloqueadas=x.bloqueadas||[];
...
async function renderizarPreviaRedistribuicao(data){const r=data.resumo||{},carteiras=(data.responsaveis_atuais||[]).filter(x=>Number(x.quantidade_elegivel||0)>0),bloqueadas=data.bloqueadas||[],novo=data.novo_responsavel||{};el.redistribuicaoResumo.innerHTML=[["Elegíveis",r.quantidade_elegivel||0,"success"],["Tratamentos iniciados",r.quantidade_tratamento_iniciado||0,"danger"],["Outros impedimentos",r.quantidade_outros_impedimentos||0,"warning"],["Para o novo responsável",r.quantidade_a_transferir||0,"neutral"]].map(x=>`<div class="preview-metric ${x[2]}"><span>${x[0]}</span><strong>${x[1]}</strong></div>`).join("");el.redistribuicaoCarteiras.innerHTML=carteiras.length?`<div class="portfolio-list">${carteiras.map(x=>`<article><div><strong>${escapeHtml(x.nome_exibicao)}</strong><span>${Number(x.quantidade_elegivel||0)} ${Number(x.quantidade_elegivel||0)===1?"indício elegível":"indícios elegíveis"}</span></div><small>${Number(x.quantidade_tratamento_iniciado||0)} iniciado(s) · ${Number(x.quantidade_outros_impedimentos||0)} outro(s) impedimento(s)</small></article>`).join("")}</div>`:'<div class="scope-empty-state">Nenhuma carteira elegível encontrada.</div>';el.redistribuicaoImpactoDestino.innerHTML=`<div class="impact-metrics"><article><span>Carga atual</span><strong>${Number(novo.carga_atual_principal||0)}</strong></article><article><span>A receber</span><strong>+${Number(novo.quantidade_a_receber||0)}</strong></article><article><span>Carga estimada</span><strong>${Number(novo.carga_estimada_principal||0)}</strong></article></div>`;const grupos=Object.entries(bloqueadas.reduce((a,x)=>{a[x.codigo]=(a[x.codigo]||0)+1;return a},{}));el.redistribuicaoDetalhes.innerHTML=`<section class="review-section"><h4>Carteiras de origem</h4>${el.redistribuicaoCarteiras.innerHTML}</section><section class="review-section"><h4>Configuração final</h4><dl class="review-definition-list"><div><dt>Novo responsável</dt><dd>${escapeHtml(novo.nome_exibicao)}</dd></div><div><dt>Quantidade a receber</dt><dd>${Number(novo.quantidade_a_receber||0)} indício(s)</dd></div><div><dt>Responsáveis anteriores</dt><dd>${el.redistribuicaoManterCheck.checked?"Manter como colaboradores":"Não manter"}</dd></div><div><dt>Prazo</dt><dd>${el.redistribuicaoNovoPrazoCheck.checked?formatarData(el.redistribuicaoNovoPrazoInput.value):"Preservar prazos atuais"}</dd></div><div class="full"><dt>Justificativa</dt><dd>${escapeHtml(el.redistribuicaoJustificativa.value)}</dd></div></dl></section>${grupos.length?`<section class="review-section"><h4>Impedimentos</h4><div class="blocked-summary">${grupos.map(([k,n])=>`<span><strong>${n}</strong> ${escapeHtml(rotuloBloqueioRedistribuicao(k))}</span>`).join("")}</div></section>`:""}`;el.redistribuicaoConfirmacaoTexto.textContent=`Revisei a prévia e confirmo a redistribuição em lote de ${Number(r.quantidade_elegivel||0)} ${Number(r.quantidade_elegivel||0)===1?"indício elegível":"indícios elegíveis"}.`;atualizarResumoRedistribuicao();}
...
function atualizarSelecionadasLote(){const n=estado.selecionadas.size;el.scopeSelectedCount.textContent=n?`${n} ${n===1?"indício selecionado":"indícios selecionados"}.`:"Nenhum indício selecionado.";el.loteQuantidade.textContent=n;el.loteQuantidadeRotulo.textContent=n?`${n} ${n===1?"indício selecionado":"indícios selecionados"}`:"Nenhum indício selecionado";el.loteSelecionadasLista.innerHTML=listaSelecionadasHtml(estado.lote.selecionadasExpandidas);el.loteAlternarSelecionadasBtn.hidden=n<=3;el.loteAlternarSelecionadasBtn.textContent=estado.lote.selecionadasExpandidas?"Recolher lista":"Ver todos";}
...
function rotuloBloqueioRedistribuicao(c){return ({TRATAMENTO_JA_INICIADO:"Tratamento já iniciado",DEMANDA_SEM_CICLO_ATIVO:"Sem ciclo ativo",CICLO_CONCLUIDO:"Ciclo concluído",DEMANDA_NAO_ESTA_PENDENTE_DE_TRATAMENTO:"Não está pendente",CICLO_NAO_PERMITE_MOVIMENTACAO:"Movimentação não permitida",CICLO_SEM_PRINCIPAL_ATIVO:"Sem responsável principal",NOVO_RESPONSAVEL_JA_E_PRINCIPAL:"Já pertence ao destino",LIMITE_DO_LOTE_EXCEDIDO:"Limite do lote excedido"})[c]||c||"Impedimento";}
