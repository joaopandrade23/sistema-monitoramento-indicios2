...
/* Painel e abas do Gestor, versao corrigida */
const GESTAO={painel:null,concluidas:[]};
const g=id=>document.getElementById(id);
async function rpcGestao(nome,args={}){const{data,error}=await sb.rpc(nome,args);if(error)throw error;return data}
function barrasGestao(id,data,rotulo){const x=g(id);if(!x)return;if(!data?.length){x.innerHTML='<div class="empty-chart">Sem dados no periodo.</div>';return}const max=Math.max(...data.map(i=>Number(i.quantidade||0)),1);x.innerHTML='<div class="bar-chart">'+data.map(i=>`<div class="bar-row"><span class="bar-label" title="${escapeHtml(i[rotulo])}">${escapeHtml(i[rotulo])}</span><span class="bar-track"><span class="bar-fill" style="width:${Math.max(Number(i.quantidade||0)/max*100,3)}%"></span></span><span class="bar-value">${Number(i.quantidade||0)}</span></div>`).join('')+'</div>'}
function colunasGestao(data){const x=g('graficoConclusoesGestao');if(!x)return;if(!data?.length){x.innerHTML='<div class="empty-chart">Sem conclusoes no periodo.</div>';return}const max=Math.max(...data.map(i=>Number(i.quantidade||0)),1);x.innerHTML='<div class="daily-chart">'+data.map(i=>`<div class="daily-column" style="height:${Math.max(Number(i.quantidade||0)/max*100,2)}%"><span>${i.quantidade||''}</span></div>`).join('')+'</div>'}
async function carregarPainelGestao(){try{const d=await rpcGestao('resumo_painel_gestor_segep',{p_data_inicial:g('painelInicio').value||null,p_data_final:g('painelFim').value||null});GESTAO.painel=d;const c=d.cards||{};g('metricasPainelGestao').innerHTML=[["Total",c.total_demandas],["Sem responsavel",c.sem_responsavel],["Aguardando inicio",c.aguardando_inicio],["Em analise",c.em_analise],["Prazo proximo",c.prazo_proximo],["Prazo vencido",c.prazo_vencido]].map(([l,v])=>`<article class="metric"><span>${l}</span><strong>${v??0}</strong><small>Estoque atual</small></article>`).join('');barrasGestao('graficoStatusGestao',d.por_status,'nome_status');barrasGestao('graficoPrazosGestao',d.por_prazo,'faixa');barrasGestao('graficoTiposGestao',d.por_tipo_indicio,'tipo_indicio');barrasGestao('graficoMovimentacoesGestao',d.movimentacoes_administrativas,'nome_movimentacao');colunasGestao(d.concluidas_por_dia);const a=d.carga_operadores||[];g('cargaOperadoresGestao').innerHTML=a.length?a.map(o=>`<article class="operator-load-card"><header><div><strong>${escapeHtml(o.nome_exibicao)}</strong><small>${escapeHtml(o.email_institucional,'')}</small></div><span class="badge badge-primary">${o.carga?.total_participacoes_ativas??0}</span></header><div class="load-metrics"><div><b>${o.carga?.como_principal??0}</b><small>Principal</small></div><div><b>${o.carga?.como_colaborador??0}</b><small>Colaborador</small></div><div><b>${o.situacao_principal?.prazos_vencidos??0}</b><small>Vencidas</small></div></div></article>`).join(''):'<div class="empty-chart">Sem operadores.</div>'}catch(e){exibirMensagem(mensagemErro(e,e.message||'Nao foi possivel carregar o painel.'),'error')}}
async function carregarConcluidasGestao(){const x=g('concluidasConteudo');x.innerHTML='<strong>Carregando...</strong>';try{const d=await rpcGestao('listar_demandas_gestao',{...parametrosListagem(),p_situacao_operacional:'ENCERRADO_INTERNAMENTE',p_pagina:1,p_tamanho_pagina:100});GESTAO.concluidas=d?.itens||[];x.innerHTML=GESTAO.concluidas.length?`<div class="table-wrap"><table><thead><tr><th>Indicio</th><th>Pessoa</th><th>Tipo</th><th>Operador</th><th>Acao</th></tr></thead><tbody>${GESTAO.concluidas.map(i=>`<tr><td>${escapeHtml(i.identificador_do_indicio)}</td><td>${escapeHtml(i.nome_atual)}<br><small>${escapeHtml(i.cpf_mascarado)}</small></td><td>${escapeHtml(i.tipo_indicio)}</td><td>${escapeHtml(i.nome_operador_principal,'Sem responsavel')}</td><td><button class="btn btn-secondary" data-consultar-concluida="${i.id_indicio}">Consultar</button></td></tr>`).join('')}</tbody></table></div>`:'<div class="table-state">Nenhuma demanda concluida.</div>'}catch(e){x.innerHTML=`<div class="status-banner error">${escapeHtml(e.message)}</div>`}}
function selecionarTabGestao(tab){const painel=g('secaoPainelGestao'),atuais=g('secaoDemandasAtuais'),concluidas=g('secaoConcluidasGestao');if(!painel||!atuais||!concluidas){console.error('HTML_GESTAO_INCOMPATIVEL',{painel,atuais,concluidas});return}document.querySelectorAll('[data-gestao-tab]').forEach(b=>{const ativo=b.dataset.gestaoTab===tab;b.classList.toggle('active',ativo);b.setAttribute('aria-selected',String(ativo))});painel.hidden=tab!=='painel';atuais.hidden=tab!=='atuais';concluidas.hidden=tab!=='concluidas';if(tab==='painel')carregarPainelGestao();if(tab==='concluidas')carregarConcluidasGestao()}
function exportarConcluidasGestao(){const c=v=>`"${String(v??'').replaceAll('"','""')}"`,rows=[["Indicio","Pessoa","CPF","Tipo","Operador"],...GESTAO.concluidas.map(i=>[i.identificador_do_indicio,i.nome_atual,i.cpf_mascarado,i.tipo_indicio,i.nome_operador_principal])];const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\ufeff'+rows.map(r=>r.map(c).join(';')).join('\r\n')],{type:'text/csv;charset=utf-8'}));a.download='demandas_concluidas.csv';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
function iniciarGestao(){const required=['secaoPainelGestao','secaoDemandasAtuais','secaoConcluidasGestao','painelInicio','painelFim','aplicarPainelBtn','exportarConcluidasBtn'];const missing=required.filter(id=>!g(id));if(missing.length){console.error('HTML_GESTAO_INCOMPATIVEL',missing);return}const end=new Date(),start=new Date(Date.now()-29*86400000);g('painelFim').value=end.toISOString().slice(0,10);g('painelInicio').value=start.toISOString().slice(0,10);document.querySelectorAll('[data-gestao-tab]').forEach(b=>b.addEventListener('click',()=>selecionarTabGestao(b.dataset.gestaoTab)));g('aplicarPainelBtn').addEventListener('click',carregarPainelGestao);g('exportarConcluidasBtn').addEventListener('click',exportarConcluidasGestao);g('concluidasConteudo').addEventListener('click',e=>{const b=e.target.closest('[data-consultar-concluida]');if(!b)return;const d=GESTAO.concluidas.find(i=>String(i.id_indicio)===b.dataset.consultarConcluida);if(d)abrirDetalhe(d)});selecionarTabGestao('painel')}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',iniciarGestao,{once:true});else iniciarGestao();
