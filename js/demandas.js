...
const estado = {
  ...
  carregando: false,
  atribuindo: false,
  prioridades: [], tiposIndicio: [], lote: { criterio: null, previa: null, assinaturaPrevia: null, etapa: 1 }, detalhe: { requisicao: 0, demanda: null, dados: null, historico: [], ciclos: [], cicloSelecionado: null, contextoCiclo: null, abaAtiva: "detalhes", cpfVisivel: false },
  concluidas: { itens: [], detalhes: new Map(), expandida: null, pagina: 1, tamanho: 20, total: 0, totalPaginas: 0 },
  redistribuicao: { criterio: null, previa: null, assinatura: null, etapa: 1 },
  buscaTimer: null,
...
function abrirLote(modo) {
  alternarMenu(false);
  estado.lote = { criterio: modo, previa: null, assinaturaPrevia: null, etapa: 1 };
  ...
  if (modo === "selecionadas") {
    el.loteTitulo.textContent = "Atribuir indícios selecionados";
    ...
    el.loteTitulo.textContent = "Atribuir indícios por tipo";
    ...
    el.loteTitulo.textContent = "Atribuir indícios por CPF";
...
function nomeCriterioLote() { return ({ selecionadas:"Indícios selecionados", tipo:"Tipo de indício", cpf:"CPF" })[estado.lote.criterio] || "Não definido"; }
function preencherResumoAtribuicao() {
  ...
  const colaboradores=idsColaboradoresSelecionados(); const previa=estado.lote.previa?.resumo||{};
  el.resumoAssistenteAtribuicao.innerHTML=`<dl class="wizard-summary-list"><div><dt>Escopo</dt><dd>${nomeCriterioLote()}</dd></div>${estado.lote.previa?`<div><dt>Elegíveis</dt><dd class="summary-success">${Number(previa.quantidade_elegivel||0)} indícios</dd></div>`:""}<div><dt>Responsável</dt><dd>${escapeHtml(principal?.nome_exibicao||"Não selecionado")}</dd></div><div><dt>Modo</dt><dd>${el.loteModoSelect.value==="COLABORATIVO"?"Colaborativo":"Individual"}</dd></div><div><dt>Colaboradores</dt><dd>${colaboradores.length?`${colaboradores.length} selecionado(s)`:"Nenhum"}</dd></div><div><dt>Prioridade</dt><dd>${escapeHtml(el.lotePrioridadeSelect.selectedOptions[0]?.textContent||"Normal")}</dd></div><div><dt>Prazo</dt><dd>${el.lotePrazoCheck.checked?formatarData(el.lotePrazoInput.value):"Não definido"}</dd></div><div><dt>Processo SEI</dt><dd>${el.loteVincularSei.checked?(el.loteSeiNumero.value||"Pendente"):"Não informado"}</dd></div></dl>`;
}
function mostrarEtapaLote(etapa){estado.lote.etapa=Math.max(1,Math.min(5,etapa));document.querySelectorAll("[data-lote-panel]").forEach(x=>x.hidden=Number(x.dataset.lotePanel)!==estado.lote.etapa);document.querySelectorAll("[data-lote-step]").forEach(x=>{const n=Number(x.dataset.loteStep);x.classList.toggle("active",n===estado.lote.etapa);x.classList.toggle("complete",n<estado.lote.etapa);x.disabled=n>estado.lote.etapa;});el.voltarLoteBtn.hidden=estado.lote.etapa===1;el.revisarLoteBtn.hidden=estado.lote.etapa===5;el.revisarLoteBtn.textContent=estado.lote.etapa===4?"Continuar para revisão":"Continuar";el.confirmarLoteBtn.hidden=estado.lote.etapa!==5;el.confirmarLoteBtn.disabled=!estado.lote.previa?.pode_confirmar||!el.loteConfirmacaoCheck.checked;preencherResumoAtribuicao();}
function validarEtapaLote(etapa){const p=parametrosLote();if(etapa===1){if(p.p_criterio==="SELECIONADAS"&&!p.p_ids_indicios?.length)throw Error("Selecione ao menos um indício.");if(p.p_criterio==="TIPO_INDICIO"&&!p.p_id_tipo_indicio)throw Error("Selecione um tipo de indício.");if(p.p_criterio==="CPF"&&String(p.p_cpf||"").replace(/\D/g,"").length!==11)throw Error("Informe um CPF com 11 dígitos.");}if(etapa===2){if(!p.p_id_usuario_operador)throw Error("Selecione o responsável principal.");if(p.p_codigo_modo==="COLABORATIVO"&&!p.p_ids_usuarios_colaboradores?.length)throw Error("Selecione ao menos um colaborador.");}if(etapa===3&&el.lotePrazoCheck.checked&&!el.lotePrazoInput.value)throw Error("Informe a data limite.");if(etapa===4&&el.loteVincularSei.checked&&!el.loteSeiNumero.value.trim())throw Error("Informe o número do processo SEI.");}
...
function mostrarEtapaRedistribuicao(etapa){estado.redistribuicao.etapa=Math.max(1,Math.min(5,etapa));document.querySelectorAll("[data-red-panel]").forEach(x=>x.hidden=Number(x.dataset.redPanel)!==estado.redistribuicao.etapa);document.querySelectorAll("[data-red-step]").forEach(x=>{const n=Number(x.dataset.redStep);x.classList.toggle("active",n===estado.redistribuicao.etapa);x.classList.toggle("complete",n<estado.redistribuicao.etapa);x.disabled=n>estado.redistribuicao.etapa;});el.voltarRedistribuicaoBtn.hidden=estado.redistribuicao.etapa===1;el.revisarRedistribuicaoBtn.hidden=estado.redistribuicao.etapa>=4;el.confirmarRedistribuicaoBtn.hidden=estado.redistribuicao.etapa!==5;el.confirmarRedistribuicaoBtn.disabled=!estado.redistribuicao.previa?.pode_confirmar||!el.redistribuicaoConfirmacaoCheck.checked;atualizarResumoRedistribuicao();}
function atualizarResumoRedistribuicao(){const atual=estado.operadores.find(x=>String(x.id_usuario)===el.redistribuicaoAtualSelect.value),novo=estado.operadores.find(x=>String(x.id_usuario)===el.redistribuicaoNovoSelect.value),r=estado.redistribuicao.previa?.resumo||{};el.resumoAssistenteRedistribuicao.innerHTML=`<dl class="wizard-summary-list"><div><dt>Escopo</dt><dd>${estado.redistribuicao.criterio==="tipo"?"Por tipo de indício":"Por CPF"}</dd></div>${estado.redistribuicao.previa?`<div><dt>Elegíveis</dt><dd class="summary-success">${Number(r.quantidade_elegivel||0)} indícios</dd></div>`:""}<div><dt>Responsável atual</dt><dd>${escapeHtml(atual?.nome_exibicao||"Não selecionado")}</dd></div><div><dt>Novo responsável</dt><dd>${escapeHtml(novo?.nome_exibicao||"Não selecionado")}</dd></div><div><dt>Responsável anterior</dt><dd>${el.redistribuicaoManterCheck.checked?"Manter como colaborador":"Não manter"}</dd></div><div><dt>Justificativa</dt><dd>${el.redistribuicaoJustificativa.value.trim().length>=10?"Informada":"Pendente"}</dd></div></dl>`;}
function validarEtapaRedistribuicao(etapa){const p=parametrosRedistribuicao();if(etapa===1){if(p.p_criterio==="TIPO_INDICIO"&&!p.p_id_tipo_indicio)throw Error("Selecione o tipo de indício.");if(p.p_criterio==="CPF"&&String(p.p_cpf||"").replace(/\D/g,"").length!==11)throw Error("Informe um CPF com 11 dígitos.");}if(etapa===2){if(!p.p_id_responsavel_atual||!p.p_id_novo_responsavel)throw Error("Selecione os dois responsáveis.");if(p.p_id_responsavel_atual===p.p_id_novo_responsavel)throw Error("O novo responsável deve ser diferente do atual.");}if(etapa===3&&el.redistribuicaoJustificativa.value.trim().length<10)throw Error("Informe uma justificativa com pelo menos 10 caracteres.");}
