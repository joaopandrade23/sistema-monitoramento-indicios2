...
function detailCard(l,v,c=""){return `<div class="detail-card ${c}"><span>${escapeHtml(l)}</span><strong>${escapeHtml(v??"Não informado")}</strong></div>`}
function detailSection(t,h){return `<h3 class="section-title">${escapeHtml(t)}</h3><div class="detail-grid-v3">${h}</div>`}
function switchDetailTab(tab){document.querySelectorAll("[data-detail-tab]").forEach(b=>b.classList.toggle("active",b.dataset.detailTab===tab));document.querySelectorAll("[data-detail-panel]").forEach(x=>x.hidden=x.dataset.detailPanel!==tab)}
async function abrirDetalhe(d){if(!d)return;estado.detalhe.demanda=d;const req=++estado.detalhe.requisicao;el.modalIdentificador.textContent=d.identificador_do_indicio||"Não informado";el.modalNome.textContent=d.nome_atual||"Não informado";el.modalCpf.textContent=d.cpf_mascarado||"CPF protegido";el.modalTipo.textContent=d.tipo_indicio||"Não informado";el.modalSituacao.textContent=rotuloSituacao(d.situacao_operacional);el.modalSituacao.className=`badge ${classeSituacao(d.situacao_operacional)}`;el.modalPrioridade.textContent=d.nome_prioridade||"Não definida";el.modalPrazo.textContent=d.prazo_em?formatarDataHora(d.prazo_em):"Sem prazo";el.modalOperador.textContent=d.nome_operador_principal||"Consultando ciclo histórico";el.modalProcessosQtd.textContent="...";el.gerenciarEquipeBtn.hidden=true;el.atribuicaoOverlay.hidden=false;document.body.style.overflow="hidden";switchDetailTab("detalhes");el.painelDetalhesGestor.innerHTML='<div class="table-state">Carregando detalhes...</div>';try{const{data,error}=await sb.rpc("obter_detalhes_demanda",{p_id_indicio:Number(d.id_indicio)});if(error)throw error;if(req!==estado.detalhe.requisicao)return;estado.detalhe.dados=data;const ciclo=data.ciclo_selecionado||data;const processos=data.processos_sei||data.processos||[];const equipe=ciclo.equipe||data.equipe||[];const principal=data.operador_principal||equipe.find(x=>x.papel_principal&&x.participacao_ativa)||{};const colaboradores=data.colaboradores||equipe.filter(x=>!x.papel_principal&&x.participacao_ativa);el.modalCpf.textContent=data.cpf_mascarado||data.cpf||d.cpf_mascarado;el.modalProcessosQtd.textContent=processos.filter(x=>x.processo_ativo!==false).length;el.modalOperador.textContent=principal.nome_exibicao||d.nome_operador_principal||"Responsável histórico não informado";el.painelDetalhesGestor.innerHTML=detailSection("Identificação",detailCard("Número do indício",data.identificador_do_indicio||d.identificador_do_indicio)+detailCard("Base de dados",data.base_de_dados||d.base_de_dados)+detailCard("Tipo de indício",data.tipo_indicio||d.tipo_indicio,"full")+detailCard("Descrição",data.descricao_indicio||"Descrição não informada.","full"))+detailSection("Pessoa",detailCard("Nome atual",data.nome_atual||d.nome_atual,"wide")+detailCard("CPF",data.cpf_mascarado||data.cpf||d.cpf_mascarado)+detailCard("Situação funcional",data.situacoes_funcionais_resumo||d.situacoes_funcionais_resumo,"full"));el.painelEquipeGestor.innerHTML=(d.situacao_operacional==="ENCERRADO_INTERNAMENTE"?'<div class="readonly-callout"><strong>Modo somente leitura.</strong> Este ciclo está concluído.</div>':"")+detailSection("Ciclo",detailCard("Número do ciclo",ciclo.numero_ciclo)+detailCard("Situação",ciclo.nome_status_ciclo||rotuloSituacao(d.situacao_operacional))+detailCard("Modo",data.modo_trabalho?.nome||data.modo_trabalho?.codigo)+detailCard("Responsável principal",principal.nome_exibicao||d.nome_operador_principal||"Não informado","wide")+detailCard("Colaboradores",colaboradores.map(x=>x.nome_exibicao).join(", ")||"Nenhum colaborador","full"));el.painelProcessosGestor.innerHTML=processos.length?`<div class="process-list">${processos.map(x=>`<article class="process-detail-card ${x.processo_ativo===false?"inactive":""}"><h3>${escapeHtml(x.numero_processo)} ${x.processo_principal?'<span class="badge badge-primary">Principal</span>':""}</h3><p>${escapeHtml(x.assunto||"Sem assunto")}</p><small>Incluído em ${formatarDataHora(x.incluido_em)}</small></article>`).join("")}</div>`:'<div class="table-state">Nenhum processo SEI vinculado.</div>';try{const{data:hist,error:he}=await sb.rpc("listar_movimentacoes_demanda_gestor",{p_id_indicio:Number(d.id_indicio),p_id_ciclo_tratamento:ciclo.id_ciclo_tratamento||d.id_ciclo_tratamento||null,p_categoria:null,p_data_inicial:null,p_data_final:null,p_pagina:1,p_tamanho_pagina:200});if(he)throw he;const rows=hist?.itens||[];el.painelHistoricoGestor.innerHTML=rows.length?`<div class="timeline">${rows.map(x=>`<article class="timeline-item"><span class="history-icon">${x.evento_automatico?"A":"H"}</span><div><div class="history-heading"><strong>${escapeHtml(x.nome_movimentacao||x.codigo_movimentacao)}</strong><time>${formatarDataHora(x.realizada_em)}</time></div><p>${escapeHtml(x.descricao||"Sem descrição.")}</p></div></article>`).join("")}</div>`:'<div class="table-state">Nenhuma movimentação registrada.</div>'}catch(e){el.painelHistoricoGestor.innerHTML=`<div class="status-banner warning">Não foi possível carregar o histórico: ${escapeHtml(e.message)}</div>`}}catch(error){console.error(error);el.painelDetalhesGestor.innerHTML=detailSection("Dados disponíveis",detailCard("Indício",d.identificador_do_indicio)+detailCard("Pessoa",d.nome_atual)+detailCard("CPF",d.cpf_mascarado)+detailCard("Tipo",d.tipo_indicio,"full"));el.modalMensagemDetalhe.textContent=mensagemErro(error,"Não foi possível carregar os detalhes completos.");el.modalMensagemDetalhe.className="status-banner modal-message error";el.modalMensagemDetalhe.hidden=false}}
...
const G={concluidas:[]};const ge=id=>document.getElementById(id);function rotuloG(v){return ({PENDENTE:"Pendente de tratamento",PENDENTE_DE_TRATAMENTO:"Pendente de tratamento",DISPONIVEL_PARA_ATRIBUICAO:"Disponível para atribuição",EM_TRATAMENTO:"Em tratamento",ENCERRADO_INTERNAMENTE:"Encerrado internamente",VENCE_EM_ATE_3_DIAS:"Vence em até 3 dias",VENCE_EM_ATE_7_DIAS:"Vence entre 4 e 7 dias",SEM_PRAZO:"Sem prazo",NAO_SE_APLICA:"Não se aplica",PRAZO_VENCIDO:"Prazo vencido"})[v]||String(v||"Não informado").replaceAll("_"," ").toLowerCase().replace(/(^|\s)\S/g,m=>m.toUpperCase())}function gBars(id,data,key){const x=ge(id);if(!x)return;if(!data?.length){x.innerHTML='<div class="empty-chart">Sem dados no período.</div>';return}const max=Math.max(...data.map(i=>+i.quantidade),1);x.innerHTML='<div class="bar-chart">'+data.map(i=>`<div class="bar-row"><span class="bar-label">${escapeHtml(rotuloG(i[key]))}</span><span class="bar-track"><span class="bar-fill" style="width:${Math.max(+i.quantidade/max*100,3)}%"></span></span><span class="bar-value">${+i.quantidade}</span></div>`).join("")+"</div>"}async function loadDash(){try{const{data,error}=await sb.rpc("resumo_painel_gestor_segep",{p_data_inicial:ge("painelInicio").value||null,p_data_final:ge("painelFim").value||null});if(error)throw error;const c=data.cards||{};ge("metricasPainelGestao").innerHTML=[["Demandas no estoque",c.total_demandas],["Aguardando atribuição",c.sem_responsavel],["Aguardando início",c.aguardando_inicio],["Em tratamento",c.em_analise],["Prazo em até 7 dias",c.prazo_proximo],["Prazo vencido",c.prazo_vencido]].map(([l,v])=>`<article class="metric"><span>${l}</span><strong>${v??0}</strong><small>Visão consolidada</small></article>`).join("");gBars("graficoStatusGestao",data.por_status,"nome_status");gBars("graficoPrazosGestao",data.por_prazo,"faixa");gBars("graficoTiposGestao",data.por_tipo_indicio,"tipo_indicio");gBars("graficoMovimentacoesGestao",data.movimentacoes_administrativas,"nome_movimentacao");const ds=data.concluidas_por_dia||[],max=Math.max(...ds.map(i=>+i.quantidade),1);ge("graficoConclusoesGestao").innerHTML=ds.length?'<div class="daily-chart">'+ds.map(i=>`<div class="daily-column" style="height:${Math.max(+i.quantidade/max*100,2)}%"><span>${i.quantidade||""}</span></div>`).join("")+"</div>":'<div class="empty-chart">Sem conclusões no período.</div>';const a=data.carga_operadores||[];ge("cargaOperadoresGestao").innerHTML=a.length?a.map(o=>`<article class="operator-load-card"><header><strong>${escapeHtml(o.nome_exibicao)}</strong><span class="badge badge-primary">${o.carga?.total_participacoes_ativas??0}</span></header><small>${escapeHtml(o.email_institucional,"")}</small><div class="load-metrics"><div><b>${o.carga?.como_principal??0}</b><small>Principal</small></div><div><b>${o.carga?.como_colaborador??0}</b><small>Colaborador</small></div><div><b>${o.situacao_principal?.prazos_vencidos??0}</b><small>Vencidas</small></div></div></article>`).join(""):'<div class="empty-chart">Sem operadores.</div>'}catch(e){exibirMensagem(mensagemErro(e,e.message||"Não foi possível carregar o painel."),"error")}}async function loadConcluidas(){const x=ge("concluidasConteudo");x.textContent="Carregando...";try{const{data,error}=await sb.rpc("listar_demandas_gestao",{...parametrosListagem(),p_situacao_operacional:"ENCERRADO_INTERNAMENTE",p_pagina:1,p_tamanho_pagina:100});if(error)throw error;G.concluidas=data?.itens||[];x.innerHTML=G.concluidas.length?`<div class="table-wrap"><table><thead><tr><th>Indício</th><th>Pessoa</th><th>Tipo</th><th>Situação</th><th>Ação</th></tr></thead><tbody>${G.concluidas.map(i=>`<tr><td>${escapeHtml(i.identificador_do_indicio)}</td><td>${escapeHtml(i.nome_atual)}<br><small>${escapeHtml(i.cpf_mascarado)}</small></td><td>${escapeHtml(i.tipo_indicio)}</td><td><span class="badge">${escapeHtml(rotuloSituacao(i.situacao_operacional))}</span></td><td><button class="btn btn-secondary" data-c="${i.id_indicio}">Consultar</button></td></tr>`).join("")}</tbody></table></div>`:'<div class="table-state">Nenhuma demanda concluída.</div>'}catch(e){x.innerHTML=`<div class="status-banner error">${escapeHtml(e.message)}</div>`}}function tabG(t){ge("secaoPainelGestao").hidden=t!=="painel";ge("secaoDemandasAtuais").hidden=t!=="atuais";ge("secaoConcluidasGestao").hidden=t!=="concluidas";document.querySelectorAll("[data-gestao-tab]").forEach(b=>b.classList.toggle("active",b.dataset.gestaoTab===t));if(t==="painel")loadDash();if(t==="concluidas")loadConcluidas()}function csv(data,name){const c=v=>`"${String(v??"").replaceAll('"','""')}"`;const a=document.createElement("a");a.href=URL.createObjectURL(new Blob(["\ufeff"+data.map(r=>r.map(c).join(";")).join("\r\n")],{type:"text/csv;charset=utf-8"}));a.download=name;a.click()}function bootG(){const end=new Date(),start=new Date(Date.now()-29*86400000);ge("painelFim").value=end.toISOString().slice(0,10);ge("painelInicio").value=start.toISOString().slice(0,10);document.querySelectorAll("[data-gestao-tab]").forEach(b=>b.onclick=()=>tabG(b.dataset.gestaoTab));ge("aplicarPainelBtn").onclick=loadDash;ge("exportarAtuaisBtn").onclick=()=>csv([["Indício","Pessoa","CPF","Tipo","Situação","Prioridade","Operador"],...estado.demandas.map(i=>[i.identificador_do_indicio,i.nome_atual,i.cpf_mascarado,i.tipo_indicio,rotuloSituacao(i.situacao_operacional),i.nome_prioridade,i.nome_operador_principal])],"demandas_atuais.csv");ge("exportarConcluidasBtn").onclick=()=>csv([["Indício","Pessoa","CPF","Tipo","Situação"],...G.concluidas.map(i=>[i.identificador_do_indicio,i.nome_atual,i.cpf_mascarado,i.tipo_indicio,rotuloSituacao(i.situacao_operacional)])],"demandas_concluidas.csv");ge("concluidasConteudo").onclick=e=>{const b=e.target.closest("[data-c]");if(b){const d=G.concluidas.find(i=>String(i.id_indicio)===b.dataset.c);if(d)abrirDetalhe(d)}};tabG("painel")}if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",bootG,{once:true});else bootG();
